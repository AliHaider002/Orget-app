...
const PaymentList = props => {
  ...
  const renderItem = ({item}) => {
    ...
    return (
      <TouchableOpacity
        onPress={() => props.navigation.navigate('PaymentEdit', {data: item})}>
        {/* {userid == item?.userid ? ( */}
      ...
      .then(querySnapshot => {
        ...
        querySnapshot.forEach(snapshot => {
          ...
          if (value && !farmname) {
            ...
            // Filter by farm only
            if (farmname === data.farm && data.route === 'payment') {
              arr.push(data);
            }
          } else if (value && farmname) {
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      // Return a cleanup function to remove the effect
    }, [userid]),
  );
  React.useEffect(() => {
    getData();
    ...
    <View style={styles.main}>
      ...
      {showfilter === false ? (
        <View
          ...
          <TouchableOpacity
            style={[styles.btn]}
            onPress={() => props.navigation.navigate('PaymentNew')}>
